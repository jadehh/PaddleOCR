...
class TextDetector(object):
    ...
    def create_predictor(self,model_dir):
        if model_dir is None:
            logging.info("not find {} model file path {}".format(model_dir))
            sys.exit(0)
        model_file_path = model_dir + "/inference.pdmodel"
        params_file_path = model_dir + "/inference.pdiparams"
        if not os.path.exists(model_file_path):
            logging.error("not find model file path {}".format(model_file_path))
            sys.exit(0)
        if not os.path.exists(params_file_path):
            logging.error("not find params file path {}".format(params_file_path))
            sys.exit(0)
...
if __name__ == '__main__':
    ...
    textSystem = TextSystem("E:\Models\Paddle2.3\ch_ppocr_mobile_v2.0_det_infer","E:\Models\Paddle2.3\ch_ppocr_mobile_v2.0_rec_infer")
    image_list = GetAllImagesPath(r"C:\Users\Administrator\Desktop\2021-10-26")
